...
export function WorkoutPlanDetailsScreen({navigation, route}: Props) {
  ...
  return <ScrollView contentContainerStyle={styles.container}><Text style={styles.eyebrow}>WORKOUT PLAN</Text><Text style={styles.title}>{plan.name}</Text><Text style={styles.description}>{plan.description}</Text><View style={styles.infoGrid}><Info label="Goal" value={labelize(plan.goal)} /><Info label="Level" value={labelize(plan.level)} /><Info label="Location" value={labelize(plan.location)} /><Info label="Frequency" value="6 days + 1 rest day" /><Info label="Duration" value={`${plan.estimatedDurationMinutes} min`} /></View><Text style={styles.section}>WORKOUT SCHEDULE</Text><View style={styles.list}>{plan.workouts.map(workout => <WorkoutCard key={workout.id} dayNumber={workout.dayNumber} onPress={workoutId => navigation.navigate('WorkoutDetails', {planId: plan.id, workoutId})} workout={workout} />)}<View style={styles.rest}><Text style={styles.restDay}>DAY 7</Text><View><Text style={styles.restTitle}>Rest & Recovery</Text><Text style={styles.restDescription}>Give your body time to recover before the next week.</Text></View></View></View></ScrollView>;
}
